...
//多维数组本质 就是数组存数组	
    ...
    //初始化房间的信息
    @Deprecated//(重构 表明这个方法已经过时 不建议使用)
	...
	public Custom getCustomByIndex(int i,int j){//通过索引访问数组
	      Custom c=null;
		  try{
		    c=rooms[i][j];
	...
	public int getCommand(int range){
		...
		while(true){
			...
			try{
				...
				command =sc.nextInt();
				if(command>0 && command <=range){//捕获异常
					break;
				}
	        }catch(InputMismatchException ex){
				System.out.println("你是不是傻！输入整数,");
			}
		}
        return command;
	}
...
//详细查询
	...
	public int  validateInputRoomNumber(){
		...
		if(i>=rooms.length||( j >=rooms[i].length)){
			...
			System.out.println("您输入的房间号码不正确");
			 continue;
		}
		 break;
	    }
	...
	*/
	...
	public static void main(String []args){
		...
		//ha.initRooms();//初始化房间的信息 不初始化都是null
